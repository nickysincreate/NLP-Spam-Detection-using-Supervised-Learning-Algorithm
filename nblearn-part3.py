...
top5Spam=sorted(spamdict, key=spamdict.get, reverse=True)[:100]

hamdict={}
hamdict=get_dict(hamFile, hamdict)
#print(len(hamdict))
top5Ham=sorted(hamdict, key=hamdict.get, reverse=True)[:100]
inter = set(top5Ham).intersection(top5Spam)
for inw in inter:
    if inw in hamdict.keys(): del hamdict[inw]
    if inw in spamdict.keys(): del spamdict[inw]
